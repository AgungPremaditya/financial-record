          ...
          >
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="payload.walletId" required>
                <option v-for="item in dataWallet" :value="item.id">
                  {{ item.name }}
                </option>
              </select>
            </div>
...
export default defineComponent({
  setup() {
    ...
    } = useApi(`transaction/${route.params.id}`);
    getTransaction().then(() => {
      payload.value.name = dataTransaction.value.name;
      payload.value.type = dataTransaction.value.type;
      payload.value.date = new Date(dataTransaction.value.date)
        .toISOString()
        .split("T")[0];
      payload.value.category = dataTransaction.value.category;
      payload.value.detail = dataTransaction.value.detail;
      payload.value.value = dataTransaction.value.value;
